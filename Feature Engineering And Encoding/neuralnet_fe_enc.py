#!/usr/bin/env python3
"""
Simple NN + Simple Feature Engineering.
Run:
    python train_nn_simple_fe.py
...
Writes:
    /mnt/data/submission_nn_simple.csv
    /mnt/data/models_nn_simple/model_fold{fold}.pt  (if PyTorch available)
Prints Macro-F1 per fold and overall OOF Macro-F1.
"""
...
# base numeric features (drop id and record_code if present)
drop_cols = []
...
print("Features after simple FE:", X_fe.shape[1])
...
# -------------------------
# Try PyTorch, else sklearn fallback
# -------------------------
use_torch = True
try:
    import torch
...
for fold, (tr_idx, val_idx) in enumerate(skf.split(X_fe, y), start=1):
    ...
    if use_torch:
        ...
        model = MLP(in_dim=X_tr_s.shape[1], out_dim=num_classes).to(device).float()
        cw = torch.tensor([len(y) / (num_classes * Counter(y)[i]) for i in range(num_classes)], dtype=torch.float32, device=device)
        ...
        oof_preds[val_idx] = np.vstack(val_preds)
        ...
        test_preds += np.vstack(test_fold_preds)
        ...
        test_preds += mlp.predict_proba(X_test_s.values)

# finalize metrics
for i, s in enumerate(fold_scores, 1):
    print(f"Fold {i} Macro-F1: {s:.4f}")
print("Mean Macro-F1:", float(np.mean(fold_scores)), "Std:", float(np.std(fold_scores)))

oof_labels = oof_preds.argmax(axis=1)
oof_f1 = f1_score(y, oof_labels, average="macro")
print("OOF Macro-F1 (overall):", oof_f1)

# average test preds and save submission
test_preds /= N_FOLDS
